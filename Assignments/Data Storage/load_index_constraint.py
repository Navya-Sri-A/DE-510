...
def createTable(conn):
        ...
        with conn.cursor() as cursor:
                ...
                print(f"Created the table based on the given data set as {TableName}")

def createIndexConstraint(conn):
        with conn.cursor() as cursor:
                cursor.execute(f"""
                        ALTER TABLE {TableName} ADD PRIMARY KEY (TractId);
                        CREATE INDEX idx_{TableName}_State ON {TableName}(State);
                """)
                print(f"Delaying the creation of Indexes and Constraints on {TableName}")
